...
func (p *Prometheus) ObserveResponseTime(status int, method, path string, observeTime float64) {
	p.reqDur.WithLabelValues(strconv.Itoa(status), method, path).Observe(observeTime)
}
